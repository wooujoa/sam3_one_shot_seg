#!/usr/bin/env python3
# SAM3 one-shot segmentation node.
#
# IMPORTANT:
# - This node is SAM3-only.
# - It does NOT import AnyGrasp / gsnet / torch.
# - It does NOT publish grasp_msgs/ObjectGrasp.
# - It only publishes segmentation/geometric input topics for AnyGrasp:
#     target_pc, object_pc, background_pc, preview/full clouds, masks, object_center.
...
class Sam3OneShotNode(Node):
    ...
    역할:
      1) RGB-D 1프레임을 받아 외부 conda env의 SAM3 inference script 실행
      2) mask를 기반으로 target/object/background point cloud 생성
      3) object center를 camera frame 기준 PointStamped로 publish
      4) AnyGrasp가 사용할 입력 토픽만 publish

    주의:
      - 이 노드는 AnyGrasp SDK를 import하지 않는다.
      - 이 노드는 ObjectGrasp 커스텀 메시지를 publish하지 않는다.
      - 최종 ObjectGrasp는 calib 노드에서만 생성한다.
    """
    ...
    def __init__(self):
        ...
        # ---------------- 3D local ROI ----------------
        # Kept for compatibility with old launch/params. Current pipeline does not hard-use these.
        self.declare_parameter('roi_min_edge_m', 0.30)
        self.declare_parameter('roi_max_edge_m', 0.60)
        self.declare_parameter('roi_scale', 2.0)

        self.declare_parameter('min_roi_points', 120)
        self.declare_parameter('min_object_points', 80)

        # ---------------- publishers ----------------
        self.declare_parameter('target_pc_topic', '/yolo/target_pc')
        self.declare_parameter('object_pc_topic', '/yolo/object_pc')
        self.declare_parameter('background_pc_topic', '/yolo/background_pc')
        self.declare_parameter('preview_cloud_topic', '/sam3/mask_pointcloud')
        self.declare_parameter('full_scene_pc_topic', '/sam3/full_scene_pc')
        self.declare_parameter('mask_image_topic', '/sam3/target_mask')
        self.declare_parameter('object_mask_image_topic', '/sam3/object_mask')
        self.declare_parameter('object_center_topic', '/sam3/object_center_camera')
        ...
        self.roi_min_edge_m = float(self.get_parameter('roi_min_edge_m').value)
        self.roi_max_edge_m = float(self.get_parameter('roi_max_edge_m').value)
        ...
        self.get_logger().info('SAM3 One-Shot Node Ready')
        ...
        self.get_logger().info(f'background_pc_topic      : "{self.background_pc_topic}"')
        self.get_logger().info(f'full_scene_pc_topic      : "{self.full_scene_pc_topic}"')
        self.get_logger().info(f'object_center_topic      : "{self.object_center_topic}"')
        self.get_logger().info('This node does NOT import AnyGrasp/gsnet/torch.')
        self.get_logger().info('This node does NOT publish ObjectGrasp.')
        self.get_logger().info('========================================')
    ...
    def rgbd_callback(self, color_msg: RosImage, depth_msg: RosImage):
        ...
        self.done = True
        self.get_logger().info('Received first synchronized RGB-D frame. Starting one-shot pipeline...')
        try:
    ...
    def run_pipeline(self, color_msg: RosImage, depth_msg: RosImage, camera_info: CameraInfo):
        ...
        self.get_logger().info(
            f'object_center_camera = ({object_center_xyz[0]:.4f}, '
            f'{object_center_xyz[1]:.4f}, {object_center_xyz[2]:.4f})'
        )
        self.get_logger().info(
            f'points | target={target_xyz.shape[0]} object={object_xyz.shape[0]} '
            f'background={background_xyz.shape[0]} full={full_xyz.shape[0]}'
        )
        ...
        self.get_logger().info(
            f'Start repeated publish: {self.publish_remaining} times, '
            f'period={self.publish_repeat_period_sec:.2f}s'
        )
    ...
    def publish_repeat_callback(self):
        if self.publish_remaining <= 0:
            if self.publish_timer is not None:
                ...
                self.publish_timer = None
            self.get_logger().info('Repeated publish finished.')
            self.schedule_shutdown()
            return
    ...
    def call_external_sam3(
        ...
        if conda_exe is None:
            conda_exe = '/home/jwg/miniconda3/bin/conda'
    ...
    def load_best_box(self, meta_npz_path: str, mask_bin: np.ndarray) -> np.ndarray:
        ...
        if ys.size == 0:
            return np.array(
                [0, 0, mask_bin.shape[1] - 1, mask_bin.shape[0] - 1],
                dtype=np.float32,
            )
    ...
    def refine_object_masks(self, obj_mask_raw: np.ndarray, roi_mask_2d: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        obj_mask_core = cv2.erode(
            mask_u8,
            core_kernel,
            iterations=max(1, self.core_erode_iterations),
        ) > 0
